    ...
    <?php if(count($items) > 0){ ?>
      <table class="table table-bordered text-center">
        ...
        <tbody>
          ...
          <tr class="<?php hprint(is_open($item) ? '' : 'close_item'); ?>">
            <td><img src="<?php hprint(IMAGE_PATH . $item['image']);?>" class="item_image"></td>
            <td><?php hprint($item['name']); ?></td>
            <td><?php hprint(number_format($item['price'])); ?>円</td>
            <td>
              <form method="post" action="admin_change_stock.php">
                <div class="form-group">
                  <input  type="number" name="stock" value="<?php hprint($item['stock']); ?>">
                  個
                </div>
